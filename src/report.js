function buildMarkdownReport({ counts, violations, total, dateStr }) {
  ...
  lines.push("");
  lines.push(`| Producten | Verboden (Annex II/III) | Zonder INCI | Conform |`);
  lines.push(`|---|---|---|---|---|`);
  lines.push(`| ${total} | ${counts.verboden} | ${counts["geen-inci"]} | ${counts.ok} |`);
  lines.push("");
  ...
  violations
    ...
    .forEach((v) => {
      ...
      v.hits.forEach((h) => {
        lines.push(
          `- **${h.inci}**${h.cas ? ` · CAS ${h.cas}` : ""} · Annex ${h.annex} · ${h.ref}${h.annexTitel ? ` — ${h.annexTitel}` : ""}${h.note ? ` — ${h.note}` : ""} (match via ${h.via})`
        );
      });
      lines.push("");
    });
